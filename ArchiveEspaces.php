...
// Restaurer l'espace si l'ID est passé en paramètre
if (isset($_GET['restore_id'])) {
    $restoreId = htmlspecialchars($_GET['restore_id']);

    // Fetch the archived space data
    $stmt = $bdd->prepare("SELECT * FROM archive_espace WHERE Id_Espace = ?");
    $stmt->execute([$restoreId]);
    $espace = $stmt->fetch();

    // Restore the record if it exists
    if ($espace) {
        $insert = $bdd->prepare("INSERT INTO espaces_verts (Id_Espace, Nom, Localisation, Superficie, Type_espace, description, disponibilite) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $insert->execute([$espace['Id_Espace'], $espace['Nom'], $espace['Localisation'], $espace['Superficie'], $espace['Type_espace'], $espace['Description'], $espace['Disponibilite']]);

        // Now delete the archived record
        $deleteQuery = $bdd->prepare("DELETE FROM archive_espace WHERE Id_Espace = ?");
        $deleteQuery->execute([$restoreId]);

        echo "<p style='color: green; text-align: center;'>Espace restauré avec succès.</p>";
    }
}
?>
...
<head>
    ...
    <style>
        ...
        .delete-button, .edit-button, .restore-button {
        ...
        .restore-button {
            background-color: #2ecc71; /* Couleur pour le bouton de restauration */
        }
...
<h1>Espaces à Louer à <?php echo $quartier; ?></h1>
  <h2 style="margin-left:46%;">Espaces Actuels</h2>
<div class="container">
    <!-- Espaces Actuels -->
...
<br><br><br><br><br>
<h2 style="margin-left:46%;">Espaces Archivés</h2>
<div class="container">
    <!-- Espaces Archivés -->
    
    <?php 
    $archiveQuery = $bdd->query('SELECT * FROM archive_espace');
    while ($archivedSpace = $archiveQuery->fetch()) {
    ?>
        <div class="card">
            <h2><?php echo htmlspecialchars($archivedSpace['Nom']); ?></h2>
            <div><strong>Quartier:</strong> <?php echo htmlspecialchars($archivedSpace['Localisation']); ?></div>
            <div><strong>Superficie:</strong> <?php echo htmlspecialchars($archivedSpace['Superficie']); ?> m2</div>
            <div><strong>Type d'espace:</strong> <?php echo htmlspecialchars($archivedSpace['Type_espace']); ?></div>
            <div><strong>Description:</strong> <?php echo htmlspecialchars($archivedSpace['Description']); ?></div>
            <div><strong>Disponibilité:</strong> <?php echo htmlspecialchars($archivedSpace['Disponibilite']); ?></div>
            <br>
            <a class="restore-button" href="?quartier=<?php echo $quartier; ?>&restore_id=<?php echo $archivedSpace['Id_Espace']; ?>" 
                onclick="return confirm('Êtes-vous sûr de vouloir restaurer cet espace ?');">Restaurer</a>
        </div>
    <?php 
    }
    ?>
...
<footer>
    <a href="choixcorbeille.php" style="color: white;">Retour</a>
