...
<head>
    ...
    <style>
        ...
        .delete-button:hover {
         ...
         .delete-buttona {
            background-color:rgb(49, 122, 42); /* Rouge */
            color: white;
            border: none;
            padding: 10px 15px;
            border-radius: 5px;
        ...
        .delete-buttona:hover {
            background-color:rgb(45, 117, 48); /* Rouge foncé au survol */
        }
...
if (isset($_GET['RESTORE'])) {
    ...
    // Fetch the archived data to restore
    $stmt = $bdd->prepare("SELECT * FROM archivedemande_utilisation WHERE Id_Demande = ?");
    $stmt->execute([$RecupRest]);
    $archivedDemande = $stmt->fetch();

    // Restore the record
    if ($archivedDemande) {
        $insertRestore = $bdd->prepare("INSERT INTO demande_utilisation (Id_Demande, Date_Demande, Id_Espace, Matricul, Description) VALUES (?, ?, ?, ?, ?)");
        $insertRestore->execute([$archivedDemande['Id_Demande'], $archivedDemande['Date_Demande'], $archivedDemande['Id_Espace'], $archivedDemande['Matricul'], $archivedDemande['Description']]);

        // Now delete the archived record
        $deleteArchived = $bdd->prepare("DELETE FROM archivedemande_utilisation WHERE Id_Demande = ?");
        $deleteArchived->execute([$RecupRest]);
    }
}
?>
...
$AffichageArchives = $bdd->query('
    SELECT * FROM archivedemande_utilisation
');
...
<div class="card">
    <div class="event-title">Id de la demande : <?php echo $archiveData["Id_Demande"]; ?></div>
    <div class="event-date">Matricul : <?php echo $archiveData["Matricul"]; ?></div>
    <div class="event-date">Date de demande : <?php echo date('d/m/Y', strtotime($archiveData["Date_Demande"])); ?></div>
    <div><strong>Description :</strong> <?php echo $archiveData["Description"]; ?></div><br><br>
    <a class="delete-buttona" href="Archive_Demande.php?RESTORE=<?php echo $archiveData["Id_Demande"]; ?>">Restaurer</a>
</div>
